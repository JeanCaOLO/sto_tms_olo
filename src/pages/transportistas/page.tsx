...
export default function TransportistasPage() {
  ...
  const { canCreate, canEdit, canDelete } = useModulePermissions('transportistas');
  ...
  const getStatusBadge = (status: string) => {
    return status === 'active'
      ? <Badge variant="success">{t('carriers.active')}</Badge>
      : <Badge variant="danger">{t('carriers.inactive')}</Badge>;
  };
  ...
  const columns: DataTableColumn<Carrier>[] = [
    ...
    { key: 'tax_id', header: t('carriers.colTaxId'), accessor: (c) => c.tax_id, sortable: true },
    { key: 'contact_name', header: t('carriers.colContact'), accessor: (c) => c.contact_name, sortable: true },
    { key: 'email', header: t('carriers.colEmail'), accessor: (c) => c.email, sortable: true },
    { key: 'phone', header: t('carriers.colPhone'), accessor: (c) => c.phone },
    { key: 'country', header: t('carriers.colCountry'), accessor: (c) => c.countries?.name ?? '', sortable: true, filterable: true },
    {
      key: 'payment_account',
      header: t('carriers.colPaymentAccount'),
      accessor: (c) => c.payment_account ?? '',
    },
    {
      key: 'softland_code',
      header: t('carriers.colSoftlandCode'),
      accessor: (c) => c.softland_code ?? '',
    },
    {
      key: 'driver_count',
      header: t('carriers.colDrivers'),
    ...
    {
      key: 'status',
      header: t('carriers.colStatus'),
      accessor: (c) => (c.status === 'active' ? t('carriers.active') : t('carriers.inactive')),
      filterable: true,
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">{t('carriers.loading')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">{t('carriers.subtitle')}</p>
        </div>
        {canCreate && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">{t('carriers.kpiTotal')}</p>
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">{t('carriers.kpiActive')}</p>
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">{t('carriers.kpiInactive')}</p>
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">{t('carriers.kpiDrivers')}</p>
      ...
      <DataTable
        ...
        columns={columns}
        getRowId={(c) => c.id}
        searchPlaceholder={t('carriers.search')}
        exportFileName="transportistas"
        emptyMessage={t('carriers.empty')}
