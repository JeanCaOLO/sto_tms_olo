...
> **⚠️ Contexto de arranque (kickoff) — parcialmente desactualizado en la lógica de negocio.**
> **Qué sigue válido:** la lista de módulos, la estructura del proyecto, el equipo y quién
> hace qué. Úsalo como el "por qué existe cada módulo".
> **Qué NO tomar al pie de la letra:** muchas *reglas de cómo funcionan* los módulos ya
> evolucionaron respecto a lo que dice aquí. La lógica de negocio vigente **se va construyendo
> en el camino**, a medida que se trabaja cada módulo — no está consolidada en un solo lugar
> todavía. Antes de implementar una regla basándote en este documento, confírmala (con el
> equipo, o contra lo ya documentado en los planes de módulo de `docs/reference/`, las
> decisiones en `docs/decisions/` y la memoria en `aidlc/spaces/default/memory/project.md`).
> Documentos que ya corrigen partes de esto: [`plan-modulo-oms.md`](plan-modulo-oms.md),
> [`analisis-sistema-tms.md`](analisis-sistema-tms.md).
...
> (complementa [`../standards/desarrollo-aws-intelix.md`](../standards/desarrollo-aws-intelix.md), que cubre los estándares técnicos/AWS).
