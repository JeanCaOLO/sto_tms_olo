...
> [`../standards/desarrollo-aws-intelix.md`](../standards/desarrollo-aws-intelix.md) §2). Sin código todavía: este archivo es el artefacto
> versionado a validar con el equipo antes de generar historias de usuario y empezar a construir.
> Dueño del módulo: **Eduardo** (ver [`contexto-proyecto-tms.md`](contexto-proyecto-tms.md) §2 y §5.1).
> Última actualización: 2026-08-24 (incorpora la reunión de trabajo del OMS — ver
> `contexto-proyecto-tms.md` §2.4).
...
**Ese no es el alcance aquí.** Según el kickoff (`contexto-proyecto-tms.md` §2), el OMS de OLO
tiene un trabajo mucho más acotado:

> "Prioriza pedidos por reglas de negocio (no FIFO)."

**Corrección importante (reunión de trabajo del OMS — ver `contexto-proyecto-tms.md` §2.4):** el
...
WMS / Torre de Control  (genera el pedido + el viaje)
          ...
          │  [verificar — contexto-proyecto-tms.md §2.4]
          ▼
    ┌────────────────┐
...
- **[SupplyX — Real-time order prioritization](https://supplyx.info/en/order-prioritization-procurement/)**
  ...
  (`contexto-proyecto-tms.md` §6.1) y con lo que ya hace Sección de Alertas en el Dashboard
...
Tal como pide [`../standards/desarrollo-aws-intelix.md`](../standards/desarrollo-aws-intelix.md) §11 ("Design system... si existe, es
...
`contexto-proyecto-tms.md` §9) y lo que separa limpiamente "motor" de "operación diaria" de
...
## 6. Boceto de arquitectura técnica (alineado a [`../standards/desarrollo-aws-intelix.md`](../standards/desarrollo-aws-intelix.md))
...
- `route_dispatch_schedule` *(nueva, submódulo 1)* — `route_id`, día(s) de la semana en que
  ...
  (`contexto-proyecto-tms.md` §2.4) — el OMS pasa a ser el dueño/CRUD de esta tabla.
...
(`contexto-proyecto-tms.md` §4) aplica igual aquí: **forzar manualmente una prioridad** es una
...
Ver el detalle completo en `contexto-proyecto-tms.md` §2.4.
...
- ✅ *¿Quién certifica humanamente un cambio antes de producción?* — **Corregido tras la Adenda
  ...
  proyecto de "certificación humana" (`contexto-proyecto-tms.md` §6.1) **no aplica** al motor de
  priorización del OMS. Esta corrección **supera** la redacción anterior de este documento y la de
  `contexto-proyecto-tms.md` §2.4, que describían una "aprobación humana obligatoria" — quedan
...
- [ ] ¿Los pedidos de Devoluciones/logística inversa entran al mismo motor de priorización del
      ...
      mapeado — `contexto-proyecto-tms.md` §9 — así que conviene decidirlo antes de que el OMS
...
- [ ] ¿El rol "Cliente" (`contexto-proyecto-tms.md` §4) debería ver algún estado de priorización
...
- **Siguiente paso:** después de la reunión, corregir/mejorar estas 5 pantallas con el feedback
  ...
  la matriz de requerimientos (§6.1 de `contexto-proyecto-tms.md`).
