...
> **Fuente de datos ejecutable:** estos mismos montos, frecuencias y parámetros están cargados
> como script SQL en [`../../sql/04_costeo_base_costa_rica.sql`](../../sql/04_costeo_base_costa_rica.sql)
> (tablas `tipos_camion`, `costos_fijos`, `costos_variables`, `depreciacion`, `parametros_globales`).
> Este `.md` es la versión legible; el `.sql` es la versión que se carga en la base. Si cambia uno,
> actualizá el otro — son la misma información en dos formatos.
