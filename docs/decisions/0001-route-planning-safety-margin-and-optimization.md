...
- **Status**: Accepted — safety margin split implemented 2026-08-12 (`src/pages/planificacion/capacity-fit.ts`); backlog items 1-7 still open
- **Date**: 2026-08-12
...
1. **Multi-vehicle order-pool splitting (fleet-level assignment, not one vehicle at a time).** Every mature product treats this as the core problem (VRP = *multi*-vehicle by definition; Split Delivery VRP literature exists specifically for demand > single-vehicle capacity). At 12–36 routes/day across a mixed fleet, planning one vehicle in isolation forces a human to manually decide which orders go to which vehicle before the tool even runs — the single highest-leverage gap.
2. **Per-stop delivery time windows.** Every researched product (Onfleet, Routific, Circuit, OptimoRoute) treats time windows as a first-class, often hard constraint. Already scoped separately in `src/lib/routePlanning/` (8am–7pm) but not merged into the active UI — flagging as the second priority to merge, not to build from scratch.
