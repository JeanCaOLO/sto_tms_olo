...
| **Matriz de distancias N×N** | `src/pages/planificacion/distance-matrix.ts` | Siempre — se calcula en memoria en cada "Optimizar paradas", no hay tabla `location_distances` real todavía | Distancia haversine × factor de rodeo 1.35 entre cada par de pedidos con coordenadas del viaje actual (no llama a Google Maps — son direcciones sintéticas) | Cuando exista la tabla real `location_distances` (backfill contra Google Maps, ver diseño en `docs/work/2026-08/`), reemplazar por un fetch a esa tabla; el prototipo con Google Maps real ya existe en `src/lib/routePlanning/build-matrices.ts`, pendiente de fusionar. |
