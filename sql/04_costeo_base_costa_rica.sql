...
-- Debe correrse manualmente en el SQL editor de Supabase (o vía CLI).
--
-- ORIGEN DE LA DATA (información de base — NO inventada):
--   Estudio de costeo de última milla de Costa Rica 2026, traído TAL CUAL desde
...
--
-- ROL EN EL TMS:
--   Fuente de datos base del costeo de flota propia (Costa Rica) que alimenta el
--   módulo de Liquidación / Tarifas. Es el desglose analítico completo (costos
--   fijos, 40 componentes variables de mantenimiento, depreciación por activo y
--   parámetros globales) — más rico que el modelo reducido actual del motor de
--   tarifas (src/lib/tarifas/cost.ts: costPerKm/depreciationPerKm/driverDaily).
--   Los montos varían por país (diésel y llantas no cuestan igual en CR que en
--   VE): esta es la base de Costa Rica. Cargar Venezuela / tercer país como
--   estudios equivalentes cuando estén disponibles.
--
-- NOTA sobre RLS: el script original de Charlie usa RLS por `auth.users`
--   (patrón Supabase directo). El resto de este TMS aísla por `organization_id`
--   vía `app_users` (ver sql/01_fase1_zonas_reglas.sql). Estas tablas de costeo
--   son CATÁLOGOS de referencia (lectura amplia, escritura autenticada); si se
--   quiere aislamiento multi-tenant, adaptar las políticas al patrón
--   `organization_id` antes de aplicar. Se deja la data tal cual; revisar las
--   políticas contra el esquema real antes de commit.
-- ============================================================================
...
drop table if exists rutas_costeo            cascade;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 7. RUTAS DE COSTEO GUARDADAS
--    (renombrada `rutas_costeo` para no colisionar con el módulo de rutas del TMS)
-- ═══════════════════════════════════════════════════════════════════════════

create table rutas_costeo (
...
create index idx_rutas_costeo_user   on rutas_costeo (user_id, created_at desc);
create index idx_rutas_costeo_camion on rutas_costeo (codigo_camion);
...
create table sku_cotizaciones (
  id              uuid primary key default gen_random_uuid(),
  ruta_id         uuid references rutas_costeo(id) on delete cascade,
  user_id         uuid references auth.users(id) on delete cascade,
...
create trigger trg_parametros_updated         before update on parametros_globales
  for each row execute function set_updated_at();
create trigger trg_rutas_costeo_updated       before update on rutas_costeo
  for each row execute function set_updated_at();
...
--     (patrón original de Charlie — ver NOTA de cabecera si se migra a
--      aislamiento por organization_id como el resto del TMS)
-- ═══════════════════════════════════════════════════════════════════════════
...
alter table rutas_costeo        enable row level security;
...
create policy "rutas costeo propias" on rutas_costeo for all
